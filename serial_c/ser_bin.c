...
int main(int argc, char **argv)
{
   ...
   while (1)
   {
      ret = getopt(argc, argv,
                  "IT:Fy:b:P:i:o:d:f:e:m:M:H:lOSt:B:rEcv:V:");
      ...
      while ( !finished  )
      {
         ...
         else /* must have something on fd_com */
         {
            ...
            if (check)
            {
               ...
               /* 2011/04/30: turn off the use of pingnum to start a new
                  block.  With the present scheme of UHDAS, files must
                  match across all data streams.  Using pingnum would
                  require communication of this info to all ser_*
                  processes, either directly or via UHDAS.  On the
                  chance that we might want to do this in the future,
                  the following block is left in the code but not in
                  the executable.
               */
               #if 0
               /* Set time_to_open to 0 as a flag to start a
               new file if the pingnum sequence goes backwards,
               indicating data collection was stopped and restarted.
               Exception: NB pingnum wraps at 65535, so don't start
               a new file in that case.
               */
               if (pingnum < last_pingnum &&
                        !(block_type == BLOCK_NB && last_pingnum == 65535))
               {
                  lf_ptr->time_to_open = 0;
                  log_ptr->time_to_open = 0;
                  binlog_ptr->time_to_open = 0;
                    /* line above was missing until 2011/04/30; */
                    /* it will be needed if the code is ever re-activated */
               }
               last_pingnum = pingnum;
               #endif
