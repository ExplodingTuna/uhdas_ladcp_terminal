...
/* ----------
   ...
   ----------
    ...
    Note: this differs from ser_asc and ser_bin in that it
    accepts SIGTERM and SIGINT as graceful stop signals, so
    it doesn't need the X via stdin.  The latter makes launching
    it as a background process in Python more complicated; if
    stdin is not handled properly, select does not block.

    The code for stopping via X is now behind #ifdef STOP_VIA_STDIN
   ...
   */
   ...
   while (!finished)
   {
      ...
      if (FD_ISSET(pipefd[0], &readfds))
      {
         ...
         if (nread == 1 && inbuf[0] == 'X')
         {
            ...
            */
            ...
            sermsg("Received signal; quitting\n", 0, 0, 1);
            continue;
         }
      }
#ifdef STOP_VIA_STDIN
      ...
      if (FD_ISSET(fd_com, &readfds))
      {
         ...
         if (accept_all || string_found(&matchstrings, buf))
         {
            clocks_read(&t_rec);
            good_msg = 1;
            if (check)
            ...
            if (good_msg)
            {
               /* Publish every good message that is selected. */
               result = zmq_broadcast(publisher, buf, nread, t_rec, time_tag);
               if (result)
               {
                  sermsg_init();
                  sermsg_cat("ERROR: zmq_broadcast returned %d\n",
                                                     result);
                  sermsg(NULL, 1, 0, 1);
               }

               /* Optionally write some lines to a file. */
               if (nstep > 0 && (accept_all ||
                                 string_selected(&matchstrings, buf, nstep)))
               {
